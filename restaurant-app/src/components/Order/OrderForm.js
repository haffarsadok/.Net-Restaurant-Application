...
export default function OrderForm(props) {
    ...
    const submitOrder = async (e) => {
        ...
        if (values.orderDetails.length === 0) {
            
            setNotify({ isOpen: true, message: 'Please add a food to your order.' });
            return; 
        }
        if (validateForm()) {
            try {
                if (values.orderMasterId === 0) {
                    const res = await createAPIEndpoint(ENDPIONTS.ORDER).create(values);
                    resetFormControls();
                    setNotify({ isOpen: true, message: 'New order is created.' });
                } else  {
                    const res = await createAPIEndpoint(ENDPIONTS.ORDER).update(values.orderMasterId, values);
                    setOrderId(0);
